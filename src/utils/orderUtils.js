...
export const normalizeOrderData = (orders) => {
  if (!Array.isArray(orders)) {
    console.warn('normalizeOrderData: Expected array, got:', typeof orders);
    return [];
  }
  ...
  return orders.map(order => {
    ...
    const normalized = {
      ...
      final_amount: Number(order.final_amount || order.totalAmount || 0),
      delivery_charges: Number(order.delivery_charges || 0),
      payment: order.payment || { method: order.paymentMethod || 'N/A', status: 'pending' },
      ...
      fname: order.fname || order.first_name || order.customer_name || '',
      ...
      city: order.city || '',
      state: order.state || '',
      pincode: order.pincode || order.postal_code || '',
